...
```
  Пакеты < 8 Б    → выход больше входа (накладные расходы заголовка)
  Пакеты 8–15 Б   → 4–37 % экономии (слабая, зависит от данных)
  Пакеты 16–64 Б  → 18–62 % экономии (оптимальный диапазон для DMD)
  Пакеты 128 Б    → ~31 % экономии (доминирует HUF, FLAG не выигрывает)
```
